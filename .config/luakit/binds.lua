...
add_binds("normal", {
    ...
    buf("^K$",                      function (w, b, m) w:prev_tab(m.count) end, {count=1}),
    buf("^J$",                      function (w, b, m) if not w:goto_tab(m.count) then w:next_tab() end end, {count=0}),
